...
class BasicAuctionInfo extends React.Component {
	...
	render() {
		...
		return (
			<div>
				<p styleName="header" className="header">Total Players are : { this.getTotalNoOfPlayers() }</p>
				<p styleName="header" className="header">Sold Players are : { this.getSoldPlayers() }</p>
				<p styleName="header" className="header">Remaining Players are : { this.getTotalNoOfPlayers() - this.getSoldPlayers() }</p>
				<form onSubmit={e => {
			        e.preventDefault()
